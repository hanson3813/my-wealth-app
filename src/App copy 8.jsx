...
const ChartTooltip = ({ active, payload, label, prefix = 'NT$' }) => {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '0.75rem', padding: '8px 14px', fontSize: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', minWidth: 140 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={() => setIsAddChartOpen(true)} style={{
          flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          padding: '11px 14px', borderRadius: '0.875rem',
          border: '1px dashed rgba(255,255,255,0.1)', color: '#9ea8bd', background: '#111',
        ...
        {snapshotData.length < 2 ? (
          ...
          <>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={snapshotData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                <defs>
                  {TREND_LINES.map(line => (
                ...
                <XAxis dataKey="date" tick={{ fill: '#a9b3c7', fontSize: 11 }} tickLine={false} axisLine={false} />
                <YAxis tick={{ fill: '#b3bed4', fontSize: 11 }} tickLine={false} axisLine={false} width={42} tickFormatter={v => `${(v / 10000).toFixed(0)}萬`} />
      ...
      <Section title="📈 各資產 ROI">
        {roiData.length === 0 ? <div className="text-center py-10 text-gray-700 text-xs">尚無可計算資產</div> : (
          <ResponsiveContainer width="100%" height={roiData.length * 36 + 20}>
            <BarChart data={roiData} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }} barSize={13}>
              <XAxis type="number" tick={{ fill: '#a6b1c6', fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={v => `${v.toFixed(0)}%`} domain={['dataMin - 5', 'dataMax + 5']} />
              <YAxis type="category" dataKey="name" tick={{ fill: '#c0c9d8', fontSize: 11 }} tickLine={false} axisLine={false} width={75} />
      ...
      <Section title="💰 月已實現盈虧">
        {pnlByMonth.length === 0 ? <div className="text-center py-10 text-gray-700 text-xs">尚無已實現盈虧記錄</div> : (
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={pnlByMonth} margin={{ top: 0, right: 0, left: 0, bottom: 0 }} barGap={4}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" vertical={false} />
              <XAxis dataKey="date" tick={{ fill: '#b4bfd5', fontSize: 11 }} tickLine={false} axisLine={false} />
              <YAxis tick={{ fill: '#c3cde1', fontSize: 11 }} tickLine={false} axisLine={false} width={36} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
              <Tooltip content={<ChartTooltip prefix={curSym} />} />
      ...
      <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '1.5rem', padding: '8px', width: '100%', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 13, color: '#adb7cd' }}>累計損益</span>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[{ label: '🪙 加密', value: totalCrypto }, { label: '📈 股票', value: totalStock }].map((item, i) => (
            <div key={i} style={{ flex: 1, background: '#0c0c0c', borderRadius: '0.875rem', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: item.value >= 0 ? '#10b981' : '#f43f5e' }}>
                    ...
                    {records.map((r, idx) => {
                      ...
                      return (
                        <motion.div key={r.id} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.03 }}
                          ...
                          <p style={{ fontSize: 14, fontWeight: 700, lineHeight: 1.3, marginBottom: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>{r.name}</p>
                          <p style={{ fontSize: 12, color: '#9fb3cf', marginBottom: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>{r.trade_date} · {cfg.label}</p>
                          {r.note && <p style={{ fontSize: 11, color: '#959eb0', marginBottom: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', width: '100%' }}>{r.note}</p>}
                          <div style={{ flex: 1 }} />
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#050505', color: '#fff', fontFamily: 'sans-serif', overflowX: 'hidden' }}>
      ...
      <main style={{
        ...
        {activePage === 'home' && (
          <>
            <div style={{ maxWidth: '36rem', margin: '0 auto', width: '100%', padding: '0 16px', boxSizing: 'border-box' }}>
            {/* 工具列：隨頁捲動消失 */}
              ...
              <div style={{ display: 'flex', backgroundColor: '#181818', borderRadius: 999, padding: 3, border: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                {['TWD', 'USD'].map(c => (
                  <button key={c} onClick={() => setDisplayCurrency(c)} style={{
                    padding: '5px 14px', borderRadius: 999, fontSize: 12, fontWeight: 900,
                    backgroundColor: displayCurrency === c ? '#fff' : 'transparent',
                    color: displayCurrency === c ? '#000' : '#c1cde2',
                    border: 'none', cursor: 'pointer', transition: 'all 0.15s',
                  }}>{c}</button>
                ))}
              </div>
              {rateDisplayStr && (
                <p style={{ flex: 1, fontSize: 12, color: '#9cb0cc', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{rateDisplayStr}</p>
              )}
              <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                <button onClick={handleExportCSV} title="匯出 CSV" style={{ padding: 8, borderRadius: 999, background: 'none', border: 'none', color: '#9ca6ba', cursor: 'pointer' }}><Download size={16} /></button>
                <button onClick={() => { fetchAll(); fetchExchangeRates(); }} style={{ padding: 8, borderRadius: 999, background: 'none', border: 'none', color: '#9ca6ba', cursor: 'pointer', animation: isRefreshing ? 'spin 1s linear infinite' : 'none' }}><RefreshCw size={16} /></button>
                <button onClick={async () => { await supabase.auth.signOut(); }} style={{ padding: 8, borderRadius: 999, background: 'none', border: 'none', color: '#9ca6ba', cursor: 'pointer' }}><LogOut size={16} /></button>
              </div>
            </div>
            ...
            <section style={{ paddingBottom: 8, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{ width: '100%', display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
                <DonutChart segments={donutSegs} size={300} strokeWidth={40}
                  ...
                  return (
                    <button key={item.type} onClick={() => setActiveTab(t => t === item.type ? 'all' : item.type)}
                      style={{
                        ...
                        padding: '8px 8px', borderRadius: '0.875rem', minWidth: 0,
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 5 }}>
                        ...
                        <span style={{ fontSize: 14, color: '#a9b4c9', whiteSpace: 'nowrap' }}>{item.label}</span>
                      </div>
                      <span style={{ fontSize: 14, fontWeight: 700, color: item.color, fontVariantNumeric: 'tabular-nums', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>
                        {item.negative ? '-' : ''}{curSym}{fmt(item.value)}
                      </span>
                      {pct && <span style={{ fontSize: 14, color: '#a9b4c9', marginTop: 2 }}>{pct}</span>}
                    </button>
                  );
                })}
              </div>
            ...
            <div style={{ marginBottom: 20, background: '#111', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '1.5rem', padding: '16px', boxSizing: 'border-box' }}>
              <p style={{ fontSize: 14, fontWeight: 900, color: '#a9b4c9', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 8 }}>投資概覽</p>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 2, background: '#0c0c0c', borderRadius: '0.875rem', padding: '10px 12px', minWidth: 0, boxSizing: 'border-box' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: '#a9b4c9' }}>投入</span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: '#a9b4c9' }}>目前</span>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 12, color: '#a9b4c9', marginBottom: 4 }}>報酬</span>
                  <span style={{ fontSize: 15, fontWeight: 900, color: stats.profit >= 0 ? '#10b981' : '#f43f5e', fontVariantNumeric: 'tabular-nums' }}>
                    {stats.roi >= 0 ? '+' : ''}{stats.roi.toFixed(1)}%
                  </span>
                  <span style={{ fontSize: 12, color: stats.profit >= 0 ? '#10b981' : '#f43f5e', fontVariantNumeric: 'tabular-nums', marginTop: 2, textAlign: 'center' }}>
                      ...
                      {group.items.map(asset => {
                        ...
                        return (
                          <motion.button key={asset.id}
                            ...
                            style={{ background: '#111', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '1rem', padding: '8px 6px', textAlign: 'left', display: 'flex', flexDirection: 'column', width: '100%', cursor: 'pointer', transition: 'border-color 0.15s', boxSizing: 'border-box', minWidth: 0, overflow: 'hidden' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 0 }}>
                              <span style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: cfg.color, display: 'inline-block', flexShrink: 0 }} />
                              {roi !== null && <span style={{ color: roiColor, fontSize: 14, fontWeight: 900 }}>{roi >= 0 ? '+' : ''}{roi.toFixed(1)}%</span>}
                            </div>
                            <p style={{ fontSize: 14, fontWeight: 700, lineHeight: 1, marginBottom: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>{titleLine}</p>
                            {amountLine && <p style={{ fontSize: 13, color: '#8796ac', marginTop: 4, marginBottom: 0, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>{amountLine}</p>}
                            {isCash && <div style={{ flex: 1 }} />}
                            {isLiability && (
                              <div style={{ marginTop: 4, marginBottom: 9, width: '100%', boxSizing: 'border-box' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                  <span style={{ fontSize: 13, color: '#8796ac' }}>已還 {(debtProg * 100).toFixed(0)}%</span>
                                  <span style={{ fontSize: 13, color: '#8796ac' }}>{costCurSym}{fmt(curDebt)}</span>
                                </div>
                                <div style={{ height: 5, backgroundColor: '#1c1c1c', borderRadius: 999, overflow: 'hidden', width: '100%' }}>
                                  <div style={{ height: '100%', width: `${debtProg * 100}%`, backgroundColor: debtBarColor, borderRadius: 999, transition: 'width 0.5s ease' }} />
                                </div>
                              </div>
                            )}
                            {!isLiability && !isCash && (
                              <div style={{ marginTop: 3, marginBottom: 4, display: 'flex', flexDirection: 'column', gap: 0, width: '100%', boxSizing: 'border-box' }}>
                                {(asset.cost_basis ?? 0) > 0 && (
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ fontSize: 13, color: '#8796ac', flexShrink: 0 }}>買</span>
                                    <span style={{ fontSize: 13, color: '#8796ac', fontWeight: 500, fontVariantNumeric: 'tabular-nums', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{costCurSym}{fmt(asset.cost_basis, 2)}</span>
                                  </div>
                                )}
                                {livePriceDisplay !== null && (
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 4 }}>
                                    <span style={{ fontSize: 13, color: '#8796ac', flexShrink: 0 }}>現</span>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, minWidth: 0 }}>
                                      <span style={{ fontSize: 13, fontWeight: 600, color: '#d4dbe7', fontVariantNumeric: 'tabular-nums', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{costCurSym}{fmt(livePriceDisplay, 2)}</span>
                                      {change24h !== null && <span style={{ fontSize: 13, fontWeight: 700, color: change24h >= 0 ? '#10b981' : '#f43f5e', flexShrink: 0 }}>{change24h >= 0 ? '▲' : '▼'}{Math.abs(change24h).toFixed(1)}%</span>}
                                    </div>
                                  </div>
                                )}
                              </div>
                            )}
                            <div style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.04)', marginBottom: 0 }} />
                            <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 0, color: isLiability ? '#f43f5e' : '#fff', fontVariantNumeric: 'tabular-nums', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', width: '100%' }}>
        ...
        {activePage === 'charts' && (
          <div style={{ maxWidth: '36rem', margin: '0 auto', paddingTop: 4, width: '100%', padding: '4px 16px 0', boxSizing: 'border-box' }}>
        ...
        {activePage === 'pnl' && (
          <div style={{ maxWidth: '36rem', margin: '0 auto', width: '100%', padding: '4px 16px 0', boxSizing: 'border-box' }}>
